        ...
        with torch.no_grad():
            ...
            for k, ema_v in self.ema.state_dict().items():
                ...
                if msd[k].dtype == torch.long:  # type Long
                    ...
                    model_v = msd[k].to(self.ema.device, non_blocking=True)
                    update_ema_jit(ema_v, model_v, self.decay_per_step, 1. - self.decay_per_step)

    def get_dict(self, model):
        with torch.no_grad():
            needs_module = hasattr(model, 'module') and not self.ema_has_module
            if not needs_module:
                new_state_dict = self.ema.state_dict()
            else:
                new_state_dict = OrderedDict()
                for k, v in self.ema.state_dict().items():
                    name = 'module.' + k
